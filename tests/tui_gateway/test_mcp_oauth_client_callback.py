...
def test_oauth_callback_rpc_forwards_iss():
    """The ``mcp.servers.oauth.callback`` RPC must forward the RFC 9207 ``iss`` it reads from
    params into the relay flow: Desktop against a remote backend drives exactly this path, and
    reading ``iss`` without passing it on leaves mcp 2.x rejecting the response whenever the
    authorization server advertised ``authorization_response_iss_parameter_supported``
    (Cloudflare, Resend)."""
    from tui_gateway import server

    flow = _make_session(session_id="sess-rpc-iss", server="hosp")

    reply = server._methods["mcp.servers.oauth.callback"](
        "request-id",
        {
            "name": "hosp",
            "session_id": "sess-rpc-iss",
            "code": "abc",
            "state": "s3cr3tstate",
